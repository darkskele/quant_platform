...
class FakeSource {
   public:
    ...
    PullResult next() {
        ...
        qp::MarketEvent ev;
        ev.base    = {.kind = qp::EventKind::Trade, .market = market_};
        ev.payload = qp::TradeEvent{.price = prices_.front()};
        prices_.pop_front();
        return ev;
    }
...
TEST(RunDataSource, PairsEachSourceWithTheMatchingSink) {
    std::tuple<FakeSource, FakeSource> sources{FakeSource{{100.0}, 0}, FakeSource{{200.0}, 2}};
    ...
    EXPECT_EQ(std::get<0>(sinks).recorded[0].base.market, 0);
    EXPECT_DOUBLE_EQ(std::get<qp::TradeEvent>(std::get<0>(sinks).recorded[0].payload).price, 100.0);

    ASSERT_EQ(std::get<1>(sinks).recorded.size(), 1u);
    EXPECT_EQ(std::get<1>(sinks).recorded[0].base.market, 2);
    EXPECT_DOUBLE_EQ(std::get<qp::TradeEvent>(std::get<1>(sinks).recorded[0].payload).price, 200.0);
}
...
TEST(RunDataSource, StopsPromptlyOnExternalStopWhenSourceNeverFinishes) {
    ...
    std::thread runner(
        [&] { run_data_source(sources, sinks, control, idx, std::chrono::milliseconds(5), 1); });
...
TEST(RunDataSource, OneLegBackpressuringNeverStallsAnotherLeg) {
    ...
    EXPECT_EQ(d0.load(std::memory_order_acquire), 1u);
