...
CostRow row(qp::SymbolId sym, qp::Market ven, Timestamp week_start_ns, double half_spread,
...
TEST(HalfSpreadLinearImpact, BuyPaysHalfSpreadWhenImpactIsZero) {
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, 0, /*half=*/5.0, /*imp=*/0.0, /*fee=*/0.0)}};
    Order o{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, SellReceivesLessByHalfSpreadWhenImpactIsZero) {
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, 0, 5.0, 0.0, 0.0)}};
    Order o{.id = 0, .symbol = 1, .side = Side::Sell, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, ImpactScalesLinearlyInQty) {
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, 0, 0.0, 2.0, 0.0)}};
    Order half{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 0.5};
    Order full{.id = 1, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, FeeIsBpsOfFilledNotional) {
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, 0, 0.0, 0.0, 4.0)}};
    Order o{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 2.0};
    auto  out = cost.price(o, 100.0, kWeekNs);
    ASSERT_TRUE(out.has_value());
...
TEST(HalfSpreadLinearImpact, MissesWhenTsPredatesEveryRow) {
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, /*week_start=*/kWeekNs, 5.0, 0.0, 4.0)}};
    Order o{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, MissesWhenSymbolNotInTable) {
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, 0, 5.0, 0.0, 4.0)}};
    Order o{.id = 0, .symbol = 2, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, LatestApplicableWeekWins) {
    ...
    Cost  cost{book,
               {
                  row(1, Market::BinanceUsdm, /*week_start=*/0 * kWeekNs, /*half=*/1.0, 0.0, 0.0),
                  row(1, Market::BinanceUsdm, /*week_start=*/1 * kWeekNs, /*half=*/2.0, 0.0, 0.0),
                  row(1, Market::BinanceUsdm, /*week_start=*/2 * kWeekNs, /*half=*/3.0, 0.0, 0.0),
              }};
    Order o{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, DoesNotSlideBetweenSymbols) {
    ...
    Cost  cost{book,
               {
                  row(1, Market::BinanceUsdm, /*week_start=*/0 * kWeekNs, 5.0, 0.0, 4.0),
                  row(2, Market::BinanceUsdm, /*week_start=*/2 * kWeekNs, 9.0, 0.0, 4.0),
              }};
    Order o{.id = 0, .symbol = 2, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, EmptyTableAlwaysMisses) {
    Book  book{kCounts};
    Cost  cost{book, {}};
    Order o{.id = 0, .symbol = 0, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
    EXPECT_FALSE(cost.price(o, 100.0, kWeekNs).has_value());
...
TEST(HalfSpreadLinearImpact, TsExactlyAtWeekStartUsesThatWeek) {
    ...
    Cost  cost{book,
               {
                  row(1, Market::BinanceUsdm, /*week_start=*/0, 1.0, 0.0, 0.0),
                  row(1, Market::BinanceUsdm, /*week_start=*/kWeekNs, 5.0, 0.0, 0.0),
              }};
    Order o{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, SpreadAndImpactCompound) {
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, 0, /*half=*/5.0, /*imp=*/1.0, 0.0)}};
    Order o{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 3.0};
    auto  out = cost.price(o, 100.0, kWeekNs);
    ASSERT_TRUE(out.has_value());
...
TEST(HalfSpreadLinearImpact, FeeAppliesToAdjustedFillPriceNotReference) {
    ...
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, 0, /*half=*/10.0, 0.0, /*fee=*/4.0)}};
    Order o{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
    auto  out = cost.price(o, 100.0, kWeekNs);
    ASSERT_TRUE(out.has_value());
...
TEST(HalfSpreadLinearImpact, WorksAcrossPriceMagnitudes) {
    ...
    Book  book{kCounts};
    Cost  cost{book, {row(1, Market::BinanceUsdm, 0, /*half=*/5.0, 0.0, 0.0)}};
    Order o{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, MultipleInstrumentsRoutedIndependently) {
    ...
    Cost  cost{book,
               {
                  row(1, Market::BinanceUsdm, 0, /*half=*/2.0, 0.0, 0.0),
                  row(3, Market::BinanceUsdm, 0, /*half=*/9.0, 0.0, 0.0),
              }};
    Order o1{.id = 0, .symbol = 1, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
    Order o3{.id = 1, .symbol = 3, .side = Side::Buy, .market = Market::BinanceUsdm, .qty = 1.0};
...
TEST(HalfSpreadLinearImpact, MoveConstructionPreservesLookups) {
    ...
    Cost  source{book,
                 {
                    row(1, Market::BinanceUsdm, 0, /*half=*/5.0, 0.0, 4.0),
                    row(1, Market::BinanceUsdm, kWeekNs, /*half=*/7.0, 0.0, 4.0),
                }};
...
TEST(HalfSpreadLinearImpact, SizeReportsTotalRowCount) {
    ...
    Cost cost{book,
              {
                  row(0, Market::BinanceUsdm, 0, 1.0, 0.0, 0.0),
                  row(1, Market::BinanceUsdm, 0, 1.0, 0.0, 0.0),
                  row(1, Market::BinanceUsdm, kWeekNs, 1.0, 0.0, 0.0),
                  row(2, Market::BinanceUsdm, 0, 1.0, 0.0, 0.0),
              }};
