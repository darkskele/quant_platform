...
TEST(BasicRiskGate, SizesOrderByDeltaAgainstCurrentPositionNotTheRawTarget) {
    ...
    auto decision =
        gate.check(qp::Intent{.symbol = 1, .venue = 0, .target_position = 5.0}, portfolio.view());
...
TEST(BasicRiskGate, ClampsTargetToMaxPositionQtyAndReportsResized) {
    ...
    auto decision =
        gate.check(qp::Intent{.symbol = 1, .venue = 0, .target_position = 8.0}, portfolio.view());
...
TEST(BasicRiskGate, OnTickFlattensTrackedPositionsOnceEquityDrawsDownPastTheThreshold) {
    ...
    EXPECT_TRUE(gate.on_tick(portfolio.view()).empty());  // establishes peak_equity_ == 0
...
TEST(BasicRiskGate, RejectsEveryCheckOnceTripped) {
    ...
    portfolio.apply_fill(
        make_fill(1, qp::Side::Buy, 1.0, /*price=*/100.0, /*order_id=*/1, /*ts=*/0, /*fee=*/1.0));
    gate.on_tick(portfolio.view());  // trips immediately: any decline clears a 0.0 threshold

    auto decision =
        gate.check(qp::Intent{.symbol = 2, .venue = 0, .target_position = 1.0}, portfolio.view());
