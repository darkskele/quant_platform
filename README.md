...
Concurrent work runs in git worktrees, one branch per worktree, sharing one `.git`.

- `tools/qp-worktree.sh <branch> [base]`: create a sibling worktree at `../quant-platform-<branch>`. Each worktree builds into its own `build/`.
