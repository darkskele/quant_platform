...
class BinanceHistoricalStream {
   public:
    BinanceHistoricalStream(Pool& pool, BinanceMarket market_path, Cadence cadence,
        ...
        : pool_(&pool),
          ...
          depth_(std::clamp<std::size_t>(prefetch_depth, 1, FileSlots::capacity())) {}

    /// Fetches this stream keeps outstanding, after clamping.
   ...
   private:
    ...
    void pump() {
        while (fetch_cursor_ < paths_.size() && outstanding() < depth_) {
