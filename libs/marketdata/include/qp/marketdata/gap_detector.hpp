...
public:
    ...
    //
    // Branch hints match steady-state reality: after the first message per
    // symbol, "already tracked" is the near-certain case, and within that,
    // an actual discontinuity is the rare one — a healthy connection hits
    // neither branch's cold path essentially ever.
    std::optional<GapInfo> check_and_record(SymbolId symbol, std::uint64_t prev_seq, std::uint64_t seq) {
        assert(symbol < kMaxSymbols && "more symbols than SequenceGapDetector::kMaxSymbols — raise the cap");
