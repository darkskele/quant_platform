...
// Live MarketDataSource: connects to Binance USD-M futures' combined
// depth-diff + aggTrade stream, normalizes each message to a MarketEvent on
// a dedicated I/O thread, and hands them to next() via an internal SPSC
// queue. Reconnects with exponential backoff on any drop.
//
...
// REST depth snapshot and Binance's align rule (find_resync_point, see
...
class LiveWebSocketSource {
public:
    explicit LiveWebSocketSource(std::vector<std::string> symbols,
                                  venue::binance::WsEndpoint   ws_endpoint   = venue::binance::kFuturesWsProduction,
                                  venue::binance::RestEndpoint rest_endpoint = venue::binance::kFuturesRestProduction);
    ~LiveWebSocketSource();
    ...
    LiveWebSocketSource(const LiveWebSocketSource&)            = delete;
    LiveWebSocketSource& operator=(const LiveWebSocketSource&) = delete;
    LiveWebSocketSource(LiveWebSocketSource&&)                 = delete;
    LiveWebSocketSource& operator=(LiveWebSocketSource&&)      = delete;
...
private:
    ...
    static_assert(!std::is_trivially_copyable_v<ResyncRequest>);  // owns std::string; SpscQueue
    ...
    struct ResyncResult {
        SymbolId                                       symbol;
        std::optional<venue::binance::DepthSnapshot>    snapshot;  // nullopt = fetch failed
    };
    static_assert(std::is_standard_layout_v<ResyncResult>);
    static_assert(!std::is_trivially_copyable_v<ResyncResult>);  // owns DepthSnapshot's vectors, same reasoning

    std::vector<std::string>     symbols_;
    venue::binance::WsEndpoint   ws_endpoint_;
    venue::binance::RestEndpoint rest_endpoint_;
    venue::binance::SymbolTable  symbol_table_;  // I/O-thread only
