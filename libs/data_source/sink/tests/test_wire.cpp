...
TEST(Wire, RoundTripsBookDiffWithLevels) {
    ...
    qp::sink::write_event(buf, ev);

    std::span<const std::byte> cursor{buf};
    auto                       decoded = qp::sink::read_event(cursor);
...
TEST(Wire, ReadsMultipleEventsBackToBackInOrder) {
    ...
    std::vector<std::byte> buf;
    qp::sink::write_event(buf, ev1);
    qp::sink::write_event(buf, ev2);
    qp::sink::write_event(buf, ev3);

    std::span<const std::byte> cursor{buf};

    auto d1 = qp::sink::read_event(cursor);
    ASSERT_TRUE(d1.has_value());
    expect_equal(ev1, *d1);

    auto d2 = qp::sink::read_event(cursor);
    ASSERT_TRUE(d2.has_value());
    expect_equal(ev2, *d2);

    auto d3 = qp::sink::read_event(cursor);
    ASSERT_TRUE(d3.has_value());
    expect_equal(ev3, *d3);

    EXPECT_TRUE(cursor.empty());
    EXPECT_FALSE(qp::sink::read_event(cursor).has_value());  // nothing left
}
