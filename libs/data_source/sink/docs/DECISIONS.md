...
**Wire format** (`write_event`/`read_event`) lives in `libs/data_source/sink` as pure,
round-trip-tested functions, no I/O — `FileRecorder` (now) and
`FileReplaySource` (Phase 1) call the *same* code, so backtest reading can't
silently drift from what the collector wrote. A truncated trailing record on
read returns `nullopt` (end-of-readable-data), not an error — also the
crash-safety contract for the read side.

**Crash safety**: periodic `ZSTD_e_flush` (not a full frame close) every ~1s
