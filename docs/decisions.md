...
## D51 — Benchmarking sticks to stock Google Benchmark (`--benchmark_repetitions` for variance, `->Threads`/param sweeps for under-load), no custom per-iteration latency sampler
A prototyped manual `chrono` p50/p99 sampler on `SpscQueue`'s ~9ns push/pop measured its own two clock calls costing ~59ns — bigger than the op — plus a 0.7-1.1ms tail traced to WSL2/Hyper-V scheduling noise, not the algorithm (`/usr/bin/time -v`: 0 involuntary context switches, 7 major page faults on an alloc-free loop) — same class of issue as the existing PMU caveat (`docs/environment.md`). `tools/bench/bench_to_md.py` renders Google Benchmark's own `--benchmark_out_format=json` (including repetition aggregates) to a root `BENCHMARKS.md`.

## ~~D52 — `CombinedTransport` (round-robin merge) deleted; `BacktestInProcessTransport` is the only `Transport` merge implementation~~
**Relocated to `docs/engine/transport/DECISIONS.md`** (the town-level doc — this is transport-city-scoped, same as D39/D42).

## D53 — No benchmarks for `apps/backtest` or `apps/collector`; `GenericLiveWebSocketSource::on_message()` also dropped
`apps/` is thin composition wiring, not its own thing to benchmark: `bench_backtest.cpp` and `bench_collector.cpp` (both real, working end-to-end benchmarks — synthetic-fixture-driven and mock-WS/HTTP-server-driven respectively) measured nothing but the same already-benched components' costs bundled together (`FileReplaySource`, `FanoutSink`, `BacktestInProcessTransport`, `Engine`, `FundingCarryStrategy`, `BasicRiskGate` for backtest; `GenericLiveWebSocketSource`, `FileRecorder` for collector) — slower and noisier than reading their individual rows, not new information. Both deleted.
`bench_live_websocket_source.cpp` (a genuine end-to-end benchmark via a real local mock WS+HTTP server, `tests/support/mock_binance_server.hpp`) also deleted: `on_message()` is private, so reaching it per-iteration meant standing up a fresh mock server pair inside the timed region — the result measured server/socket/handshake setup cost, not the parse/gap-check/queue-push logic actually worth measuring. Needs a throughput-style harness (wall-clock over N messages against one long-lived server, not GBench's per-iteration model) — a real redesign, not a fix to the existing shape; revisit later rather than ship a number that measures the wrong thing.
