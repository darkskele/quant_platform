...
void BM_CheckApprovesWhenFlat(benchmark::State& state) {
...
BENCHMARK(BM_CheckApprovesWhenFlat);

// check() clamps to max_position_qty — the Resized path.
void BM_CheckClampsAndResizes(benchmark::State& state) {
    qp::Portfolio portfolio;
...
BENCHMARK(BM_CheckClampsAndResizes);
...
void BM_OnTickNoDrawdown(benchmark::State& state) {
    qp::Portfolio portfolio;
    BasicRiskGate gate{BasicRiskGateConfig{}};
